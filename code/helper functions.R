...
# rets <- c(-0.02, .16, .31, .17, -.11, .21, .26, -0.03, .38)
# r <- rets; mar <- 0.01 (should get dd = 0.04333)
downside_deviation <- function(r, mar = 0) {
  diff <- r - mar
  neg_rets <- diff[diff < 0]
  sq <- neg_rets^2
  avg_sq <- sum(sq) / length(diff)
  dd <- sqrt(avg_sq)
}



sparkline_widget <- function(x, ticker, type = "line") {
