...
mod_backtest_backtest_ui <-
    function(id, ticker_choices, monthly_rets) {
        ...
        tagList(
            ...
            column(
                width = 8,
                box(strong(div("Decision Rules:", class = "text-muted")),
                    wellPanel(
                        p("Here you can explore custom decision rules directly 
                          below. Simply enter the condition in a format similar 
                          to these:"
                        ),
                        withTags(ul(
                            li("if_else(x < 0, 1, 0)"),
                            ...
                            li(
                                "if_else(SMA(x, 3) > 0.01 & stats::lag(x, 1) > 
                                0 & runSD(x, 3) < 0.1, 1, 0)"
                            )
                        ))
                    )
        ...
        moduleServer(id,
                     function(input, output, session) {
                         ...
                         output$backtest_plot <- renderPlot({
                             ...
                             strat_returns <-
                                 (merge.xts(invested_asset_returns(), cash_rets) * stats::lag(wts) - 0.0005) %>%
                                 rowSums() %>%
                             ...
                             rets <-
                                 merge.xts(asset_returns_filt()[, isolate(input$invested_ticker)], strat_returns) %>% na.omit()
