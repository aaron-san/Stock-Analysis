...
mod_returns_returns_server <- function(id, monthly_rets, brush_data) {
    moduleServer(id, function(input, output, session) {
        ...
        output$returns_plot <- renderPlot({
            ...
            # input$w4 + input$w5 == 100, "The portfolio weights
            # must sum to 100%!"))
            ##########
            # min_date <- function() {"2000-12-31"}
            # input <- list(tickers = c("SPY", "MDY", "IWM"))
            ##########
            ...
            monthly_rets %>% 
                ...
                theme(legend.title = element_blank())
        }) %>% bindCache(input$tickers, min_date())
                     ...
                     output$click_info <- renderDataTable({
                         # Because it's a ggplot2, we don't need to supply 
                         # xvar or yvar; if this
