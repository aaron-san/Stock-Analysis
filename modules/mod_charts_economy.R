...
fluidRow(
    box(
        width = 6,
        plotlyOutput(ns("gdp"), height = "300px"),
        footer = checkboxGroupButtons( #selectizeInput(
            ns("lookback_gdp"),
            label = "Lookback (years):",
            choices = c(1, 3, 5, 10, 20, 50),
            selected = 5,
            # value = 3,
            # min = 1,
            # max = 30,
    ...
    box(
        width = 6,
        plotlyOutput(ns("yield_curve_spread"), height = "300px"),
        footer = selectizeInput(
            ns("lookback_curve_spread"),
            label = "Lookback (months):",
            selected = 12,
            choices = c(3, 6, 9, seq(12, 12*floor(length(dates_yields)/12), by = 12))#,
            # width = "40%"
...
fluidRow(
    box(
        width = 6,
        plotlyOutput(ns("inflation"), height = "300px"),
        footer = selectizeInput(
            ns("lookback_inflation"),
            label = "Lookback (No. years):",
            choices = seq(1, 55, by = 3), #1:(length(dates_inflation) - 1),
            selected = 5#,
            # width = "50%"
        )
    ),
    box(
        ...
        footer = selectizeInput(
            ...
            selected = 12,
            choices = c(3, 6, 9, seq(12, 12*floor(length(dates_bond_yields)/12), by = 12))#,
            # width = "50%"
        )
    ) 
),
    ...
    box(width = 6,
            ...
            selectInput(width = 150,
                        ...
                        choices = 
                            ...
                            # Cash = "SHV"),
                            monthly_rets %>% select(2, 5, 8, 10, 13) %>% colnames(),
                        selected = "^GSPC"),
            ...
            footer = tagList(
                ...
                withTags(
                    ul(
                        li("It seems that when SPY does poorly, MBB and TLT are the best alternatives."),
                        li("It seems that when LQD does poorly, MBB and TLT are the best alternatives."),
                        li("It seems that when GSG does poorly, AGG, MBB, and TLT are the best alternatives."),
                        li("It seems that when MDY does poorly, MBB and TLT are the best alternatives."),
                        li("It seems that when IWM does poorly, MBB and TLT are the best alternatives."),
                        li("It seems that when EFA does poorly, TLT is the best alternative."),
                        li("It seems that when EEM does poorly, MBB and TLT are the best alternatives."),
                        li("It seems that when TLT does poorly, most of the other assets are good alternatives.")
                )
                )
            )
    )
    ...
    function(id, yields_monthly, gdp, inflation, bond_yields) {
        moduleServer(id,
                     function(input, output, session) {
                         ...
                         output$yield_curve_spread <- renderPlotly({
                             
                             ####
                             # For testing
                             # input <- list(lookback_curve_spread = 72)
                             ####
                             ...
                             start_date <- 
                                 ...
                                 dplyr::pull(date) %>%
                                 xts::last() %>%
                                 subtract(months(as.numeric(input$lookback_curve_spread)))
                             ...
                             p <- 
                                 data_for_plot %>% 
                                 pivot_wider(names_from = symbol, values_from = yield) %>% 
                                 mutate(spread = `20 year` - `2 year`) %>% 
                                 mutate(avg_10yr = mean(spread)) %>% 
                                 select(date, spread, avg_10yr) %>% 
                                 pivot_longer(-date) %>% 
                                 drop_na() %>% 
                                 ggplot(aes(x = date, y = value, color = name)) +
                                 geom_line() +
                                 theme_minimal() +
                                 labs(title = "Yield curve spread (20yr - 2yr)",
                                      y = "%")
                                 
                             ggplotly(p) %>% config(displayModeBar = FALSE)
                         }) %>% bindCache(input$lookback_curve_spread)
                         ...
                         output$gdp <- renderPlotly({
                             ...
                             data_for_plot <- 
                                 ...
                                 dplyr::mutate(fill_color = ifelse(change > 0, "steelblue", "red"),
                                               sma = TTR::SMA(change, as.numeric(input$lookback_gdp))) %>% # n-year MA
                                 dplyr::filter(date >= start_date)
                         ...
                         output$inflation <- renderPlotly({
                             ...
                             data_for_plot <- 
                                 inflation %>%
                                 # mutate(level = level * 4) %>% 
                                 mutate(sma = TTR::SMA(level, as.numeric(input$lookback_inflation))) %>% # n-year MA
                                 filter(date >= start_date) %>% 
                                 mutate(across(level, ~signif(.x, digits = 2)))
                                 
                             
                             p <-
                                 data_for_plot %>%
                                 ggplot(aes(x = date, y = level)) +
                                 geom_bar(#aes(text = map(paste('<b>Date:</b>', date, '<br>', '<b>level:</b>', level), HTML)
                                          #   ),
                                     width = 1,     
                                     stat = "identity", show.legend = FALSE, fill = "#666666") +
                                 geom_point(size = 2) + 
                                 geom_hline(aes(yintercept = xts::last(sma)), color = "red", size = 0.3,
                                            linetype = 2, alpha = 0.7, show.legend = FALSE) +
                                 scale_x_date(date_breaks = "years", date_labels = "%Y") +
                                 scale_y_continuous() + #labels = scales::deci_format()) +
                                 labs(title = "Annualized Inflation rate",
                         ...
                         }) %>% bindCache(input$lookback_inflation)
                         ...
                         }) %>% bindCache(input$lookback_bond_yields)
                         ...
                         output$cor_plot <- renderPlot(
                             ...
                             ######
                             # for testing
                             # input <- list(weak_ticker = "IWM")
                             ######
                             
                                 ggcorrplot::ggcorrplot(
                                 cor(monthly_rets %>% select(2, 5, 8, 10, 13) %>% drop_na() %>% 
                                         # select(c(IWM, DPZ, AGG)) %>%
                                         filter(., .data[[input$weak_ticker]] < 0)),
                                 hc.order = TRUE,
                                 type = "lower",
                                 colors = c("#6D9EC1", "white", "#E46726"),
                                 lab = TRUE
                             )
                         ) %>% bindCache(input$weak_ticker)
